...
# Pre-cache titan images and other static data
TITAN_TYPE_IMAGE_URLS = {
    "goofy grinning": "https://i.ibb.co/dJ6J58s0/image.jpg",
    "potbellied": "https://i.ibb.co/XkMw0Xt5/image.jpg",
    "bearded": "https://i.ibb.co/7J8S4s6v/image.jpg",
...
# Pre-calculate difficulty levels for faster lookup
DIFFICULTY_BY_LEVEL = {level: "Easy" if level < 8 else ("Normal" if level < 15 else "Hard") for level in range(1, 50)}
DEFAULT_AREAS = ["Trost District", "Karanes District", "Shiganshina District"]

# Cache frequently used strings
BATTLE_BUTTON_TEXT = "⚔️ Battle"
EXPLORE_MESSAGE_TEMPLATE = (
    "<code>-------------------------</code>\n"
    "📍 <b>{name} Lvl ({level})</b>\n"
    "<b>has blocked your way{image}</b>\n"
    "<code>-------------------------</code>\n"
)

# Pre-generate common titans for ultra-fast access
CACHED_TITANS = {}

def init_titan_cache():
    """Pre-generate titans for levels 1-30 to avoid generation overhead"""
    global CACHED_TITANS
    for level in range(1, 31):
        for difficulty in ["Easy", "Normal", "Hard"]:
            cache_key = f"{level}_{difficulty}"
            if cache_key not in CACHED_TITANS:
                CACHED_TITANS[cache_key] = _generate_cached_titan(level, difficulty)

def _generate_cached_titan(level: int, difficulty: str) -> dict:
    """Generate and cache titan data as dict for ultra-fast access"""
    from database.models import generate_titan_name, generate_titan_hp, generate_titan_xp
    
    # Use deterministic seed for consistent caching
    seed = level * 1000 + hash(difficulty) % 1000
    titan_random = random.Random(seed)
    
    # Generate base stats
    actual_level = max(1, level + titan_random.randint(-2, 2))
    name = generate_titan_name(difficulty)
    max_hp = generate_titan_hp(actual_level, difficulty)
    xp_reward = generate_titan_xp(actual_level, difficulty)
    
    # Pre-calculate image URL
    name_lower = name.lower()
    image_url = None
    for type_name, url in TITAN_TYPE_IMAGE_URLS.items():
    ...
    return {
        ...
        'image_url': image_url,
        'abilities': [],
        'spawn_areas': DEFAULT_AREAS,
        'drop_table': {},
        'min_level_requirement': actual_level
    }

# Initialize cache on import
init_titan_cache()

def get_cached_titan(player_level: int, unlocked_areas: Optional[List] = None) -> Titan:
    """Get pre-cached titan for ultra-fast response"""
    difficulty = DIFFICULTY_BY_LEVEL.get(player_level, "Hard")
    cache_key = f"{player_level}_{difficulty}"
    
    # Get from cache or generate fallback
    if cache_key in CACHED_TITANS:
        cached_data = CACHED_TITANS[cache_key]
        # Add some randomness to level
        actual_level = max(1, cached_data['level'] + random.randint(-1, 1))
        cached_data['level'] = actual_level
    else:
        # Fallback for edge cases
        cached_data = _generate_cached_titan(player_level, difficulty)
    
    # Convert to Titan object
    now = datetime.now(timezone.utc)
    return Titan(
        name=cached_data['name'],
        level=cached_data['level'],
        max_hp=cached_data['max_hp'],
        abilities=cached_data['abilities'],
        created_at=now,
        difficulty=cached_data['difficulty'],
        spawn_areas=unlocked_areas or cached_data['spawn_areas'],
        drop_table=cached_data['drop_table'],
        xp_reward=cached_data['xp_reward'],
        min_level_requirement=cached_data['min_level_requirement']
    )

# Helper function to check if a user is in battle
def _is_in_battle(user_id_str: str) -> bool:
    """Check if a user is in battle - centralized helper function"""
    try:
        from game.battle_system import active_battles
...
async def _reply_error(update: Update, message: str):
    """Helper to reply with error messages - optimized."""
    try:
        if update.message:
            await update.message.reply_text(message)
        elif update.callback_query:
...
async def explore(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if location and location.startswith("Decision_") and location in TRAVEL_MAP:
        directions = TRAVEL_MAP[location]
        keyboard = [
            [InlineKeyboardButton(dir, callback_data=f"travel_decision_{dir.strip().lower()}")]
            for dir in directions.keys()
    ...
    reply_text = EXPLORE_MESSAGE_TEMPLATE.format(
    ...
    try:
        ...
        await _reply_error(update, "An error occurred. Please try again.")
        return
    ...
    # Create actual Titan object for database (in background)
    titan = Titan(
        name=titan_data['name'],
        level=titan_level,
...
async def _setup_timeout(user_id, context, sent_message):
    """Setup timeout for titan encounter"""
    key = f"titan_timeouts_{user_id}"
    if key not in context.bot_data:
        context.bot_data[key] = []
...
async def _handle_additional_background_tasks(update, context, user_id, user_id_str, db, player_data):
    """Handle verification, missions, and other background tasks"""
    try:
...
async def _handle_spam_ban(user_id, update, context):
    """Handle spam banning in background"""
    try:
        db = context.bot_data.get("db")
        if not db:
            return
        ...
        logger.error(f"Spam ban error: {e}")
